...
class gen_inv(models.TransientModel):
    ...
    type = fields.Selection(
        [
            ("in", "Customer"),
            ("out", "Supplier"),
            ("cus_credit_note", "Customer Credit Note"),
            ("ven_credit_note", "Vendor Credit Note"),
        ],
        string="Type",
        required=True,
        default="in",
    ...
    import_prod_option = fields.Selection(
        [("name", "Name"), ("code", "Code"), ("barcode", "Barcode")], string="Import Product By ", default="name"
    )
        ...
        if self.sequence_opt == "custom":
            if self.type == "in":
                invoice_search = invoice_obj.search(
                    [("custom_name", "=", values.get("invoice")), ("move_type", "=", "out_invoice")]
                )
            elif self.type == "out":
                invoice_search = invoice_obj.search(
                    [("custom_name", "=", values.get("invoice")), ("move_type", "=", "in_invoice")]
                )
            elif self.type == "cus_credit_note":
                invoice_search = invoice_obj.search(
                    [("custom_name", "=", values.get("invoice")), ("move_type", "=", "out_refund")]
                )
            else:
                invoice_search = invoice_obj.search(
                    [("custom_name", "=", values.get("invoice")), ("move_type", "=", "in_refund")]
                )
        else:
            if self.type == "in":
                invoice_search = invoice_obj.search(
                    [("invoice_name", "=", values.get("invoice")), ("move_type", "=", "out_invoice")]
                )
            elif self.type == "out":
                invoice_search = invoice_obj.search(
                    [("invoice_name", "=", values.get("invoice")), ("move_type", "=", "in_invoice")]
                )
            elif self.type == "cus_credit_note":
                invoice_search = invoice_obj.search(
                    [("invoice_name", "=", values.get("invoice")), ("move_type", "=", "out_refund")]
                )
            else:
                invoice_search = invoice_obj.search(
                    [("invoice_name", "=", values.get("invoice")), ("move_type", "=", "in_refund")]
                )
        if invoice_search:
                    ...
                    if any_special:
                            ...
                            if many2x_fields.ttype in ["many2one", "many2many"]:
                                if many2x_fields.ttype == "many2one":
                                    if values.get(i):
                                        ...
                                        if fetch_m2o.id:
                                            inv_id.update({technical_fields_name: fetch_m2o.id})
                                        else:
                                            raise ValidationError(
                                ...
                                if many2x_fields.ttype == "many2many":
                                    ...
                                    if values.get(i):
                                        if ";" in values.get(i):
                                            ...
                                            m2m_value_lst.append(m2m_id.id)
                                    inv_id.update({technical_fields_name: m2m_value_lst})
                            else:
                                raise ValidationError(
                        ...
                        if normal_fields.id:
                            if normal_fields.ttype == "boolean":
                                inv_id.update({normal_details: values.get(i)})
                                ...
                                inv_id.update({normal_details: float_value})
                                ...
                                inv_id.update({normal_details: int_value})
