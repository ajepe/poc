            ...
            for row_no in range(sheet.nrows):
                ...
                if row_no <= 0:
                    ...
                    if self.product_details_option == "from_product":
                        values.update({"code": line[0].split(".")[0], "quantity": line[1]})
        ...
        if self.product_details_option == "from_product":
            if self.import_prod_option == "barcode":
                ...
                product_obj_search = self.env["product.product"].search(
                    [("default_code", "=", values["code"])], limit=1
                )
            else:
                                    ...
                                    if many2x_fields.ttype in ["many2one", "many2many"]:
                                        if many2x_fields.ttype == "many2one":
                                            if values.get(i):
                                                ...
                                                if fetch_m2o.id:
                                                    po_order_lines.update({technical_fields_name: fetch_m2o.id})
                                                else:
                                        ...
                                        if many2x_fields.ttype == "many2many":
                                            ...
                                            if values.get(i):
                                                if ";" in values.get(i):
                                                    ...
                                                    m2m_value_lst.append(m2m_id.id)
                                            po_order_lines.update({technical_fields_name: m2m_value_lst})
                                    else:
                                        raise ValidationError(
                                    ...
                                    if normal_fields.ttype == "boolean":
                                        ...
                                        if str(values.get(i)) == "TRUE":
                                            ...
                                            if int(str(values.get(i))) == 1:
                                                boolean_check = True
                                        po_order_lines.update({normal_details: boolean_check})
                                    elif normal_fields.ttype == "char":
                                        po_order_lines.update({normal_details: values.get(i)})
                                    elif normal_fields.ttype == "float":
                                        if values.get(i) == "":
                                            float_value = 0.0
                                        else:
                                            float_value = float(values.get(i))
                                        po_order_lines.update({normal_details: float_value})
                                        ...
                                        po_order_lines.update({normal_details: int_value})
