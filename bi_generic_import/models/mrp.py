...
class gen_mrp(models.TransientModel):
    ...
    import_prod_option = fields.Selection(
        [("barcode", "Barcode"), ("code", "Code"), ("name", "Name")], string="Import Product By ", default="name"
    )
    import_material_prod_option = fields.Selection(
        [("barcode", "Barcode"), ("code", "Code"), ("name", "Name")],
        string="Import Material Product By ",
        default="name",
    )
    ...
    def make_bom(self, values):
        ...
        if self.import_prod_option == "barcode":
            product_obj_search = self.env["product.template"].search(
                [("barcode", "=", values.get("product_tmpl").split(".")[0])], limit=1
            )
        elif self.import_prod_option == "code":
            product_obj_search = self.env["product.template"].search(
                [("default_code", "=", values.get("product_tmpl"))], limit=1
            )
        else:
            product_obj_search = self.env["product.template"].search(
                [("name", "=", values.get("product_tmpl"))], limit=1
            )
                            ...
                            if any_special:
                                    ...
                                    if many2x_fields.ttype in ["many2one", "many2many"]:
                                        if many2x_fields.ttype == "many2one":
                                            if values.get(i):
                                                ...
                                                if fetch_m2o.id:
                                                    bom_search[0].update({technical_fields_name: fetch_m2o.id})
                                                else:
                                        ...
                                        if many2x_fields.ttype == "many2many":
                                            ...
                                            if values.get(i):
                                                if ";" in values.get(i):
                                                    ...
                                                    m2m_value_lst.append(m2m_id.id)
                                            bom_search[0].update({technical_fields_name: m2m_value_lst})
                                    else:
                                        raise ValidationError(
                                ...
                                if normal_fields.id:
                                    if normal_fields.ttype == "boolean":
                                        bom_search[0].update({normal_details: values.get(i)})
                                        ...
                                        bom_search[0].update({normal_details: float_value})
                                        ...
                                        bom_search[0].update({normal_details: int_value})
            ...
            for i in main_list:
                ...
                if normal_details.startswith("x_"):
                    ...
                    if any_special:
                        ...
                        if many2x_fields.id:
                            if many2x_fields.ttype == "many2one":
                                if values.get(i):
                                    ...
                                    if fetch_m2o.id:
                                        bom_id.update({technical_fields_name: fetch_m2o.id})
                                    else:
                                        raise ValidationError(
                                            _('"%s" This custom field value "%s" not available in system')
                                            % (i, values.get(i))
                            ...
                            if many2x_fields.ttype == "many2many":
                                ...
                                if values.get(i):
                                    if ";" in values.get(i):
                                        ...
                                        m2m_value_lst.append(m2m_id.id)
                                bom_id.update({technical_fields_name: m2m_value_lst})
                        else:
                            raise ValidationError(
                        ...
                        if normal_fields.id:
                            if normal_fields.ttype == "boolean":
                                bom_id.update({normal_details: values.get(i)})
                            elif normal_fields.ttype == "char":
                                bom_id.update({normal_details: values.get(i)})
                            elif normal_fields.ttype == "float":
                                if values.get(i) == "":
                                    ...
                                    float_value = float(values.get(i))
                                bom_id.update({normal_details: float_value})
                            elif normal_fields.ttype == "integer":
                                if values.get(i) == "":
                                    int_value = 0
                                else:
                                ...
                                bom_id.update({normal_details: int_value})
                            elif normal_fields.ttype == "selection":
                                bom_id.update({normal_details: values.get(i)})
                            elif normal_fields.ttype == "text":
                                bom_id.update({normal_details: values.get(i)})
    ...
    def make_bom_line(self, values, bom_id):
        ...
        if self.import_material_prod_option == "barcode":
            product_obj_search = self.env["product.product"].search(
                [("barcode", "=", values.get("product").split(".")[0])]
            )
