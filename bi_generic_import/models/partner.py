...
class gen_partner(models.TransientModel):
    ...
    partner_option = fields.Selection(
        [("create", "Create Partner"), ("update", "Update Partner")], string="Option", required=True, default="create"
    )
        ...
        if type(val) == dict:
            if val.get("country"):
                ...
                state_search = self.env["res.country.state"].search(
                    [("name", "=", val.get("state")), ("country_id", "=", country_search.id)]
                )
            else:
            ...
            if state_search:
                if len(state_search.ids) > 1:
                    raise ValidationError(
                        "Multiple States of name %s found. Please provide Country." % val.get("state")
                    )
                else:
                ...
                if not val.get("country"):
                    ...
                    states = self.env["res.country.state"].search(
                        [["code", "=", val.get("state")[:3]], ["country_id", "=", country.id]]
                    )
                    if states:
                        if not val.get("state_code"):
                            raise ValidationError(
                                _(
                                    'Another State with code "%s" found in country "%s". Please Provide State Code in XLS/CSV.'
                                    % (val.get("state")[:3], country.name)
                                )
                            )
                        else:
                    ...
                    state = self.env["res.country.state"].create(
                        {"name": val.get("state"), "code": code, "country_id": country.id}
                    )
                    return state.id
        else:
            if val[9]:
                ...
                state_search = self.env["res.country.state"].search(
                    [("name", "=", val[6]), ("country_id", "=", country_search.id)]
                )
            else:
            ...
            if state_search:
                ...
                if not val[9]:
                    ...
                    states = self.env["res.country.state"].search(
                        [["code", "=", val[6][:3]], ["country_id", "=", country.id]]
                    )
                    if states:
                        if not val[7]:
                            raise ValidationError(
                    ...
                    state = self.env["res.country.state"].create(
                        {"name": val[6], "code": code, "country_id": country.id}
                    )
                    return state.id
                    ...
                    if any_special:
                        ...
                        if normal_fields.id:
                            if normal_fields.ttype == "boolean":
                                vals.update({normal_details: values.get(i)})
                                ...
                                vals.update({normal_details: float_value})
                                ...
                                vals.update({normal_details: int_value})
                    ...
                    if i == 0:
                        ...
                        if self.partner_option == "create":
                            ...
                            if values.get("type").lower() == "company":
                                if values.get("parent"):
                                    raise ValidationError(
                                        _("You can not give parent if you have select type is company")
                                    )
                                type = "company"
                            else:
                                type = "person"
                                    ...
                                    if count > 19:
                                        ...
                                        if normal_details.startswith("x_"):
                                            ...
                                            if any_special:
                                                    ...
                                                    if many2x_fields.ttype == "many2one":
                                                        if values.get(i):
                                                            ...
                                                            if fetch_m2o.id:
                                                                ...
                                                                raise ValidationError(
                                                                    _(
                                                                        '"%s" This custom field value "%s" not available in system'
                                                                    )
                                                                    % (i, values.get(i))
                                                                )
                                                    if many2x_fields.ttype == "many2many":
                                                        ...
                                                        if values.get(i):
                                                            if ";" in values.get(i):
                                                                ...
                                                                m2m_value_lst.append(m2m_id.id)
                                                        search_partner.update({technical_fields_name: m2m_value_lst})
                                                else:
                                                    raise ValidationError(
                                                        _('"%s" This m2x custom field is not available in system')
                                                        % technical_fields_name
                                                    )
                                            else:
                                                normal_fields = self.env["ir.model.fields"].search(
                                                    [("name", "=", normal_details), ("model_id", "=", model_id.id)]
                                                ...
                                                if normal_fields.id:
                                                    if normal_fields.ttype == "boolean":
                                                        search_partner.update({normal_details: values.get(i)})
                                                    elif normal_fields.ttype == "char":
                                                        search_partner.update({normal_details: values.get(i)})
                                                    elif normal_fields.ttype == "float":
                                                        if values.get(i) == "":
                                                            ...
                                                            float_value = float(values.get(i))
                                                        search_partner.update({normal_details: float_value})
                                                    elif normal_fields.ttype == "integer":
                                                        if values.get(i) == "":
                                                            int_value = 0
                                                        else:
                                                            try:
                                                                ...
                                                                raise ValidationError(
                                                                    _(
                                                                        "Wrong value %s for Integer field %s"
                                                                        % (values.get(i), normal_details)
                                                                    )
                                                                )
                                                        search_partner.update({normal_details: int_value})
                                                    elif normal_fields.ttype == "selection":
                                                        search_partner.update({normal_details: values.get(i)})
                                                    elif normal_fields.ttype == "text":
                                                        search_partner.update({normal_details: values.get(i)})
                                                    ...
                                                    raise ValidationError(
                                                        _('"%s" This custom field is not available in system')
                                                        % normal_details
                                                    )
                            else:
                                ...
                                if count > 19:
                                    ...
                                    if normal_details.startswith("x_"):
                                        ...
                                        if any_special:
                                            ...
                                            if many2x_fields.id:
                                                if many2x_fields.ttype == "many2one":
                                                    if line[count]:
                                                        ...
                                                        if fetch_m2o.id:
                                                            search_partner.update({technical_fields_name: fetch_m2o.id})
                                                        else:
                                                            raise ValidationError(
                                                                _(
                                                                    '"%s" This custom field value "%s" not available in system'
                                                                )
                                                                % (technical_fields_name, line[count])
                                                            )
                                                if many2x_fields.ttype == "many2many":
                                                    m2m_value_lst = []
                                                    if line[count]:
                                                        if ";" in line[count]:
                                                            ...
                                                            if not m2m_id:
                                                                raise ValidationError(
                                                                    ...
                                                                    % (technical_fields_name, m2m_names)
                                                                )
                                                            m2m_value_lst.append(m2m_id.id)
                                                    search_partner.update({technical_fields_name: m2m_value_lst})
                                            else:
                                            ...
                                            if normal_fields.id:
                                                if normal_fields.ttype == "boolean":
                                                    search_partner.update({normal_details: line[count]})
                                                elif normal_fields.ttype == "char":
                                                    search_partner.update({normal_details: line[count]})
                                                elif normal_fields.ttype == "float":
                                                    if values.get(i) == "":
