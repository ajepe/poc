...
class import_pickingss(models.TransientModel):
    ...
    picking_type_code = fields.Selection(
        [("incoming", "Vendors"), ("outgoing", "Customers"), ("internal", "Internal")], related="picking_type_id.code"
    )
    ...
    def make_picking_line(self, values, pick_id):
        ...
        if values.get("lot") != "":
            ...
            if not product_lot:
                raise ValidationError(
                    _('"%s" Lot is not available for "%s" Product.') % (values.get("lot"), values.get("product"))
                )
        else:
            product_lot = False
