...
class VendorPricelist(models.TransientModel):
    ...
    import_prod_option = fields.Selection(
        [("name", "Name"), ("code", "Code"), ("barcode", "Barcode")], string="Select Product By", default="name"
    )
    import_prod_variant_option = fields.Selection(
        [("name", "Name"), ("code", "Code"), ("barcode", "Barcode")], string="Select Product Variant BY", default="name"
    )
    ...
    def make_pricelist(self, values):
        ...
        if self.import_prod_option == "barcode":
            ...
            product_template_search = product_templ_obj.search(
                [("default_code", "=", values["product_template"])], limit=1
            )
        ...
        if self.import_prod_variant_option == "barcode":
            ...
            product_variant_search = product_variant_obj.search(
                [("default_code", "=", values["product_variant"])], limit=1
            )
        ...
        if len(product_variant_search) == 0:
            if not product_variant_search:
                raise ValidationError(
                    _(
                        "%s This product variant is not available in system, please enter valid product."
        ...
        if len(product_template_search) == 0:
            if not product_template_search:
                raise ValidationError(
                    _(
                        "%s This product template is not available in system, please enter valid product."
                        % (values["product_template"])
                    )
                )
        ...
        if product_variant_search and product_template_search:
            for variant in product_variant_search:
                if variant.product_tmpl_id.id != product_template_search.id:
                    raise ValidationError(
                        _("The %s is not a variant of %s." % (variant.name, product_template_search.name))
                    )
...
class SalePricelist(models.TransientModel):
    ...
    compute_type = fields.Selection(
        [("both", "Fixed/Percentage"), ("formula", "Formula")], string="Compute Type", default="both"
    )
    ...
    def make_pricelist(self, values):
        ...
        search_pricelist = prod_pricelist_obj.search([("name", "=", values.get("name"))])
        ...
        if search_pricelist:
            for sup in search_pricelist:
                if sup.currency_id.name == values.get("currency"):
                    ...
                    raise ValidationError(
                        _('Currency is different for "%s" . Please define same.') % values.get("name")
                    )
        else:
    ...
    def make_pricelist_line(self, values, pricelist_id):
        ...
        if compute_type == "both":
            ...
            if values.get("based_on"):
                ...
                raise ValidationError(
                    _(
                        "Please fill the 'Based On' column in CSV or XLS file, if you want to import pricelist using formula."
                    )
                )
